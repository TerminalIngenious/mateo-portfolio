...
const empty: Project = {
  ...
  tags: [], image_url: '', size: 'half', order_index: 0,
}
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerLogo}><span>M</span>G — Admin</div>
        <div className={styles.headerRight}>
          ...
          <a href="/" className={styles.viewSite}>Voir le site →</a>
        </div>
      </header>
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            <div className={styles.row}>
              ...
              <div className={styles.group}>
                ...
                <input className={styles.input} value={form.type}
                  ...
                  placeholder="Clip musical, Corporate..." required />
              </div>
            </div>
            <div className={styles.group}>
            ...
            <div className={styles.row}>
              <div className={styles.group}>
                ...
                <select className={styles.input} value={form.size}
            ...
            <div className={styles.formActions}>
              {editing && (
                <button type="button" className={styles.cancelBtn}
                  onClick={() => { setForm(empty); setEditing(null); setTagsInput('') }}>
                  Annuler
                </button>
              )}
              <button type="submit" className={styles.saveBtn} disabled={saving}>
                {saving ? <span className={styles.spinner} /> : editing ? 'Mettre à jour' : 'Ajouter le projet'}
              </button>
            </div>
          </form>
        </section>
            ...
            {projects.map(p => (
              <div key={p.id} className={styles.projectItem}>
                <div className={styles.projectInfo}>
                  ...
                  <span className={styles.projectTags}>{p.tags?.join(', ')}</span>
                </div>
                <div className={styles.projectActions}>
                  <button className={styles.editBtn} onClick={() => handleEdit(p)}>Modifier</button>
